...
def get_traffic_by_city(city_name: str):
    ...
    city_df["congestion_ratio"] = (
        city_df["current_speed_kmh"] / city_df["free_flow_speed_kmh"]
    )

    latest_speed = float(latest_row["current_speed_kmh"])
    latest_congestion = float(
        latest_row["current_speed_kmh"] / latest_row["free_flow_speed_kmh"]
    )
    ...
    # Peak congestion (top 5)
    peak = city_df.sort_values("congestion_ratio").head(5)
